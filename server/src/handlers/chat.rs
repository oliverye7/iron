...
async fn make_openai_call(
    ...
    // TODO EXPONENT: make this function return a stream, so the frontend can consume the stream via the websocket
    // TODO EXPONENT: however, also need to make sure that the function calling this receives all the output at once, 
    ...
    let system_prompt = fs::read_to_string("system_prompt.txt")
                            .expect("Failed to read system_prompt.txt");
    let user_message = chatContext.context.lock().expect("Failed to acquire lock");
